...
def get_event_dispatcher(
    ...
    :param err: error handler. Receives an error message that, if it comes from this module, will contain
        at least an 'error' and a 'message' field
    :param handlers: Handlers for the event. For each event type to handle, it should have a function that
        takes the event as an argument. Only one handler per event is permitted. Exceptions are not caught.
        If no specific handler is available and a '__default__' handler is included in the handlers, that
